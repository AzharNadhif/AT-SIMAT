...
describe('AT-CORE-0040', () => {
    ...
    describe('AT-CORE-0040 (01-02)', async () => {
        it('Receiving Surat Jalan', async () => {
            ...
            const { sjNumber } = await OutSJPage.createSJ(bagNumber2, "TRUCK(B 1234 TEST)", "KURIR ANTER (DPKS230)", soft);
            await NavigationFlow.logout();
            ...
            await ReceiveSuratPage.btnReceive.waitForClickable({ timeout:5000 });
            await ReceiveSuratPage.btnReceive.click();
            await ReceiveSuratPage.receiveSurat(sjNumber, bagNumber2, null, soft);
            await NavigationFlow.logout();

            // Validate SJ
            await NavigationFlow.loginAndNavigateToInventoryBagJOG();
